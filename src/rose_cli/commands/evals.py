...
def list_evals(limit: int = typer.Option(20, "--limit", "-l", help="Number of evaluations to show")):
...
def run_eval(
    ...
    try:
            ...
            try:
                ...
                if found_eval:
                    ...
                    if eval_id.lower() in standard_datasets:
                        ...
                        eval_obj = client.evals.create(
                            name=eval_id.lower(),
                            data_source_config={"type": "stored_completions", "metadata": {"source": eval_id.lower()}},
                            testing_criteria=testing_criteria,
                        )
                        eval_id = eval_obj.id
        ...
        request_data = {"data_source": {"type": "stored_completions", "model": model}}
        ...
        run = client.evals.runs.create(eval_id=eval_id, **request_data)
...
def show_results(
    ...
    try:
        ...
        if run.status == "completed" and hasattr(run, "results"):
            ...
            if samples:
                try:
                    samples_iter = client.evals.runs.output_items.list(eval_id=eval_id, run_id=run_id, limit=10)
...
def delete_eval(eval_id: str, confirm: bool = typer.Option(False, "--yes", "-y", help="Skip confirmation")):
