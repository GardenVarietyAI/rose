...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate fused model on benchmarks using lm-eval.")
    parser.add_argument("--model", required=True, help="Path to fused model directory.")
    ...
    logger.info("Evaluating fused model: %s", model_path)
    logger.info("Tasks: %s", args.tasks)

    config: dict[str, Any] = {
        "--model": "hf",
        "--model_args": f"pretrained={model_path},device=mps",
        "--tasks": args.tasks,
